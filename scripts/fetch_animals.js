...
// ---------------------------------------------------------------------------
// Slovak name map + continent/ocean translations
// ---------------------------------------------------------------------------
const NAME_SK = {
  ...
  'Flying lemur': 'Letucha', 'Tarsier': 'Nártoun',
  ...
  'Sumatran tiger': 'Tiger sumatriansky',
  'Asian water buffalo': 'Byvol vodný',
  'Bali myna': 'Škorec balský',
  ...
  'European polecat': 'Tchor tmavý',
  'Common kestrel': 'Sokol myšiar',
  ...
  'Common octopus': 'Chobotnica obyčajná',
  ...
  'Mountain tapir': 'Tapír horský',
  'Matamata': 'Matamata',
  'South American sea lion': 'Lachtan juhoamerický',
  'Leatherback sea turtle': 'Korytnačka kožatá',
  ...
  'Bilby': 'Bilbi', 'Eastern grey kangaroo': 'Klokan sivý',
  'Tree kangaroo': 'Klokan stromový',
  'Wallaby': 'Valabi', 'Common brushtail possum': 'Kusus líščí',
  'Spotted-tailed quoll': 'Kunovček škvrnitý',
...
const CONTINENT_SK = {
  'Africa': 'Afrika', 'Asia': 'Ázia', 'Europe': 'Európa',
  'North America': 'Severná Amerika', 'South America': 'Južná Amerika',
  'Australia': 'Austrália', 'Antarctica': 'Antarktída',
};

function translateContinent(value) {
  if (Array.isArray(value)) return value.map(v => CONTINENT_SK[v] || v);
  return CONTINENT_SK[value] || value;
}
...
async function main() {
  ...
  for (let i = 0; i < ANIMALS_SEED.length; i += BATCH) {
    ...
    await Promise.all(batch.map(async seed => {
      ...
      results.push({
        id,
        label:     NAME_SK[seed.name] || seed.name,
        imageUrl,
        continent: translateContinent(seed.continent),
